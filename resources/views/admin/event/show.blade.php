<x-layouts.admin title="Detail Event">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button class="btn btn-sm btn-primary mr-2" onclick="openEditModal(this)"
                                            data-id="{{ $ticket->id }}" data-tipe-id="{{ $ticket->tipe_tiket_id }}" data-tipe-name="{{ $ticket->tipeTiket?->nama }}"
    ...
    <dialog id="edit_ticket_modal" class="modal">
        <form method="POST" class="modal-box">
            ...
            <div class="form-control mb-4">
                ...
                <select name="tipe_tiket_id" id="edit_tipe" class="select select-bordered w-full" required>
                    <option value="" disabled selected>Pilih Tipe Ticket</option>
                    @foreach($tipeTikets as $tt)
                        <option value="{{ $tt->id }}">{{ $tt->nama }}</option>
                    @endforeach
    ...
    <script>
        ...
        function openEditModal(button) {
            const id = button.dataset.id;
            const tipeId = button.dataset.tipeId;
                const tipeName = button.dataset.tipeName;
                const harga = button.dataset.harga;
                const stok = button.dataset.stok;

            const form = document.querySelector('#edit_ticket_modal form');
            document.getElementById("edit_ticket_id").value = id;
            document.getElementById("edit_tipe").value = tipeId;
